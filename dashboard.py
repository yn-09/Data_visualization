...
def draw_recommendation_matrix(df_data, selected_occupation=None):
    ...
    ax.text(4.1, 4.5, 'QUICK WINS', fontsize=8, fontweight='bold', color='green', ha='center')
    ax.text(4.1, 1.5, 'COPILOT ASSIST', fontsize=8, fontweight='bold', color='blue', ha='center')
    ax.text(1.9, 4.5, 'TECH CHALLENGES', fontsize=8, fontweight='bold', color='orange', ha='center')
    ax.text(1.9, 1.5, 'HUMAN CORE', fontsize=8, fontweight='bold', color='red', ha='center')
...
def draw_top_desire_tasks(df_data, selected_occupation=None, top_n=10):
    ...
    sns.barplot(x=r_repetitive + r_error + r_stress, y='Task_Short', data=df_top, label='Stress', color='#f39c12', ax=ax)
    sns.barplot(x=r_repetitive + r_error, y='Task_Short', data=df_top, label='Human Error', color='#3498db', ax=ax)
    sns.barplot(x=r_repetitive, y='Task_Short', data=df_top, label='Repetitive', color='#2ecc71', ax=ax)
...
def draw_gap_analysis_dumbbell(df_data, selected_occupation=None):
    ...
    ax.scatter(gap_data['Automation Desire Rating'], gap_data['Skill_Short'], 
               color='#e17055', s=60, label='Nhân sự IT (Worker Desire)', zorder=3)
    ...
    ax.legend(bbox_to_anchor=(0.5, 1.10), loc='upper center', ncol=2, frameon=True, fontsize=8)
    sns.despine(left=True, bottom=True)
    plt.tight_layout()
